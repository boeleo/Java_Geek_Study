...
```
// Part2：实现queue的模式，只需把`Destination destination = session.createQueue(queueName);` 改为 `Destination destination = session.createTopic(topicName);`即可。
// 比较
